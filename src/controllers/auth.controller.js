...
export const login = async (req, res) => {
    try {
        ...
        if (!usuario) 
            return res.status(400).json({ message: 'Credenciales inválidas 2 '+email });

        //paso 1 verificar si el usuario existe por su email
        //const usuario = await usuarioModel.findUsuarioByEmail(email);
        //if(!usuario){
        ...
        if (!esValida) 
            return res.status(401).json({ message: 'Credenciales inválidas 1 '+password });
